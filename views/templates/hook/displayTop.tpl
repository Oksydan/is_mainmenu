        ...
        <ul class="js-main-menu-list main-menu__list row align-items-center justify-content-center mb-0">
            {foreach $menu as $menuElement}
                {if in_array($menuElement.type, ['category', 'custom', 'cms'])}
                    <li
                      class="main-menu__item main-menu__item--{$menuElement.depth} position-static col-auto {if $menuElement.has_children}js-main-menu-item{/if}"
                      data-id="{$menuElement.id}"
                    >
                        <a href="{$menuElement.url}"
                           class="main-menu__link main-menu__link--{$menuElement.depth}"
                        >
                            {$menuElement.title}
                        </a>
